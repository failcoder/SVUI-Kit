...
local function TradeFrameStyle()
	...
	TradeFrameInset:Die()
	TradeFrameTradeButton:SetStyle()
	TradeFrameCancelButton:SetStyle()
	SV.API:Set("CloseButton", TradeFrameCloseButton, TradeFrame.Panel)
	TradePlayerInputMoneyFrameGold:SetStyle()
	TradePlayerInputMoneyFrameSilver:SetStyle()
	TradePlayerInputMoneyFrameCopper:SetStyle()
	...
	for i = 1, 7 do 
		...
		if Y and Z then
			...
			Y.bg:SetStyle("[INSET]Transparent")
			...
			Z.bg:SetStyle("[INSET]Transparent")
